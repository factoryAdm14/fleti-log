...
<div class="card card-body card-chat justify-content-center px-0 pb-0 border 0 h-100 chat_box_state" id="">
        ...
        <div
            ...
            <div class="media align-items-center gap-10 flex-grow-1">
                ...
                <div class="chat_ib media-body title-color">
                    ...
                    <div class="fs-12 opacity-50 d-block" id=""> {{ $customer?->phone ?? 'N/A' }}
                    </div>
                </div>
            </div>
            <div class="dropdown">
                ...
                <ul class="dropdown-menu dropdown-menu-lg dropdown-menu-right primary" style="">
                    <li>
                        <a class="dropdown-item fs-16 py-3"
                            href="{{ route('admin.customer.show', ['id' => $customer?->id]) }}"
                            target="_blank">{{ translate('Customer Details') }}</a>
                        <a class="dropdown-item fs-16 py-3"
                            href="{{ route('admin.customer.show', ['id' => $customer?->id, 'tab' => 'trips']) }}"
    ...
    <div class="card-body p-3 overflow-y-auto conversation flex-grow-1 msg_history d-flex flex-column-reverse"
        ...
        @foreach ($conversations as $conversation)
            @if ($conversation->user->user_type == CUSTOMER)
                            ...
                            <div class="received_msg" data-bs-toggle="tooltip" data-bs-title="{{ formatCustomDateForTooltip($conversation?->created_at) }}">
                                <div class="received_withdraw_msg">
                                    <div class="message-text-section rounded mt-1">
                                        @if ($conversation?->serviceRequest)
                                            <div class="border rounded p-2 mb-2 bg-white">
                                                <div class="fw-semibold mb-1">{{ translate('Service request') }}</div>
                                                <div class="fs-12"><strong>{{ translate('Type') }}:</strong> {{ $conversation->serviceRequest->service_type === 'delivery' ? translate('Delivery') : translate('Ride') }}</div>
                                                <div class="fs-12"><strong>{{ translate('Origin') }}:</strong> {{ $conversation->serviceRequest->origin_address }}</div>
                                                <div class="fs-12"><strong>{{ translate('Destination') }}:</strong> {{ $conversation->serviceRequest->destination_address }}</div>
                                                @if ($conversation->serviceRequest->notes)
                                                    <div class="fs-12"><strong>{{ translate('Notes') }}:</strong> {{ $conversation->serviceRequest->notes }}</div>
                                                @endif
                                            </div>
                                        @endif
                                        <p class="m-0 pb-1">
            ...
            <form class="mt-4 chatting-messages-ajax-form" enctype="multipart/form-data" method="POST">
                ...
                <div class="position-relative d-flex">
                    ...
                    <button class="btn send-btn fw-bold fs-24 px-4 typing" type="submit" id="msgSendBtn" disabled
                        data-channel-id="{{ $channelId }}" data-customer-id="{{ $customer->id }}">
                        <i class="tio-send"></i>
                    </button>
