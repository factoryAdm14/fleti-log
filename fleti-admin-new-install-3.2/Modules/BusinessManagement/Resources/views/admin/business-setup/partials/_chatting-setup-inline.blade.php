<div class="card border-0 mb-3">
    <div class="card-header d-flex flex-md-nowrap flex-wrap align-items-center justify-content-between gap-3">
        ...
        <div class="max-w300 w-100 border py-2 px-3 rounded rounded d-flex align-items-center justify-content-between mt-2">
            <label for="customerChattingSetupStatus" class="fs-14 lh-22 d-block cursor-pointer text-dark">{{ translate('Customer Chat') }}</label>
            <label class="switcher cmn_focus rounded-pill">
                <input class="switcher_input collapsible-card-switcher update-business-setting"
                       id="customerChattingSetupStatus" tabindex="2"
                       type="checkbox"
                       name="customer_chatting_setup_status"
                       data-name="customer_chatting_setup_status"
                       data-type="{{CHATTING_SETTINGS}}"
                       data-url="{{route('admin.business.setup.update-business-setting')}}"
                       data-icon=" {{($settings->firstWhere('key_name', 'customer_chatting_setup_status')->value ?? 0) == 1 ? dynamicAsset('public/assets/admin-module/img/chatting-off.png') : dynamicAsset('public/assets/admin-module/img/chatting-on.png')}}"
                       data-title="{{translate('Are you sure')}}?"
                       data-sub-title="{{($settings->firstWhere('key_name', 'customer_chatting_setup_status')->value?? 0) == 1 ? translate('Do you want to turn OFF Chatting Option for customer & admin')."? ": translate('Do you want to turn ON Chatting Option for customer & admin')."? "}}"
                       data-confirm-btn="{{($settings->firstWhere('key_name', 'customer_chatting_setup_status')->value?? 0) == 1 ? translate('Turn Off') : translate('Turn On')}}"
                    {{($settings->firstWhere('key_name', 'customer_chatting_setup_status')->value?? 0) == 1? 'checked' : ''}}
                >
                <span class="switcher_control"></span>
            </label>
        </div>
...
<ul class="nav d-inline-flex nav--tabs bg-transparent nav--tabs__style2 p-1 rounded bg-white mb-3">
    ...
    <li class="nav-item text-capitalize">
        <a href="{{route('admin.business.setup.chatting-setup.index',CUSTOMER)}}" class="nav-link {{Request::is('admin/business/setup/chatting-setup/customer') ? 'active' : ''}}">{{translate("Customer")}}</a>
    </li>
    <li class="nav-item">
